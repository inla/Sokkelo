...
```
##Algoritmit
Abstraktissa Algoritmi-luokassa on metodi kasiteltavanSolmunNaapurit(), jossa solmun koordinaattien vieruskoordinaatit tutkitaan ja sellaiset, jotka eivät ole sokkelon ulkopuolella tai esteitä, lisätään listaan. Metodissa on for-silmukka, joka suoritetaan neljä kertaa, ja sen sisällä listalle lisäys-operaatio, jonka aikavaativuus on vakio. Listalle tulee enintään neljä alkiota, joten listan kasvatus-operaatiota ei tarvitse käyttää. Siten solmun naapurien etsintä on vakioaikaista. Metodissa maaliLoytyi() on while-silmukka, joka suoritetaan enintään reitin pituuden verran.
